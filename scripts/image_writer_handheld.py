...
class ImageRectWriter(object):
    ...
    def callback2(self, image_msg, mask_msg, rect_msg):
        ...
        self.write_annotated_data(cv_img, mask_img, [x, y, w, h])
    ...
    def write_annotated_data(self, cv_img, mask_img, im_rect):
        ...
        if self.__counter < self.__frame_write:
            ...
            with open(self.__json_path + str(self.__counter).zfill(8) + '__labels.json', 'w') as f:
                json.dump(json_label, f)        
    ...
    def json_encoding(self, rect, image_fn, mask_fn):
        ...
        anno['label_type'] = 'box'
        anno['label_class'] = self.__obj_name[:-1]
        label['labels'] = [anno]
        label['complete'] = 'null'
    ...
    def subscribe(self):
        if self.__write_rect:
            ...
            ts = message_filters.ApproximateTimeSynchronizer([image_sub, mask_sub, rect_sub], 10, 10)
...
def main(argv):
    try:
        rospy.init_node('image_rect_writer', anonymous = True)
        irw = ImageRectWriter()
        rospy.spin()
